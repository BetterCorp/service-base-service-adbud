...
export class Plugin extends CPlugin<MyPluginConfig> {
  async init(): Promise<void> {
    await this.onReturnableEvent<IAdBudRequest<undefined>, ICustomerData>(
      null,
      "get-customer",
      async (data?) => {
    ...
    await this.onReturnableEvent<
      IAdBudRequest<undefined>,
      ICustomerSuggestionData
    >(null, "get-suggestions", async (data?) => {
      if (Tools.isNullOrUndefined(data)) throw "Undefined Data!";
      const adbudClient = new adbud(
        data!.auth.host,
        data!.auth.username,
    ...
    await this.onReturnableEvent<IAdBudRequest<undefined>, ISearchTermDatum[]>(
      null,
      "get-search-terms",
      async (data?) => {
        if (Tools.isNullOrUndefined(data)) throw "Undefined Data!";
    ...
    await this.onReturnableEvent<
      IAdBudRequest<{ term: string; wanted: boolean }>,
      ISetTermData
    >(null, "set-search-term", async (data?) => {
      if (Tools.isNullOrUndefined(data)) throw "Undefined Data!";
      const adbudClient = new adbud(
        data!.auth.host,
        data!.auth.username,
      ...
      await adbudClient.login();
      return await adbudClient.setSearchTerm(
        data!.data.term,
        data!.data.wanted
      );
    });
    ...
    await this.onReturnableEvent<
      IAdBudRequest<IAdBudRequestStats>,
      IDashboardData
    >(null, "get-stats", async (data?) => {
      if (Tools.isNullOrUndefined(data)) throw "Undefined Data!";
      const adbudClient = new adbud(
        data!.auth.host,
        data!.auth.username,
    ...
    await this.onReturnableEvent<
      IAdBudRequest<IGeoTargetRequest>,
      Array<IGeoTarget>
    >(null, "get-geo-target", async (data?) => {
      if (Tools.isNullOrUndefined(data)) throw "Undefined Data!";
      const adbudClient = new adbud(
        data!.auth.host,
        data!.auth.username,
